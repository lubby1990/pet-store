...
//					.prepareStatement(sql);
			...
			jdbcTemplate.query(sql,objs,new RowCallbackHandler(){
		      ...
		      /*  ,new RowMapper() {
				...
				public Object mapRow(ResultSet rs, int rowNum)
					...
					getUser.setGender(StringUtils.isNotBlank(rs.getString(5)) ? Gender
							.valueOf(rs.getString(5)) : null);
			...
			while (rs.next()) {
				...
				String address = rs.getString(6);
				String nation = rs.getString(7);
				Gender genderE = StringUtils.isNotBlank(gender) ? Gender
						.valueOf(gender) : null;
				user = new User(userId, uName, uPassword, nickName, genderE,
						address, nation);
				list.add(user);
...
//		Connection conn = dbUtil.getConnection();
		try {
			...
			Object[] ojbs = {user.getUserName(),user.getPassword(),user.getNickName(),user.getGender().name(),user.getAddress(),user.getNation()};
